...
export const Dashboard = (): JSX.Element => {
  ...
  return (
    <div className={`w-full min-h-screen bg-[#2a4149] ${isMobile ? 'p-2' : 'p-3 sm:p-4 md:p-6'} overflow-hidden`}>
      <div className="max-w-[1400px] mx-auto" style={{ width: '100%', maxWidth: '100%' }}>
        {/* Header Section - Responsive */}
        <div className={`flex flex-col lg:flex-row lg:items-start lg:justify-between ${isMobile ? 'gap-2 mb-2' : 'gap-4 mb-6'}`}>
          <div className="flex-shrink-0">
            <h1 className={`[font-family:'Roboto',Helvetica] font-bold text-[#66dbe1] ${isMobile ? 'text-lg leading-tight' : 'text-2xl sm:text-3xl md:text-[38px] tracking-[0] leading-tight sm:leading-[48.1px]'}`}>
          ...
          {isMobile ? (
            <Collapsible open={isFiltersOpen} onOpenChange={setIsFiltersOpen} className="w-full">
              <CollapsibleTrigger asChild>
                <button className="w-full flex items-center justify-between px-2 py-1.5 bg-[#ffffff24] rounded-md border border-solid border-[#dae0e633] hover:bg-[#ffffff30] transition-colors">
                  <div className="flex items-center gap-2">
                    <Filter className="w-3.5 h-3.5 text-[#66dbe1]" />
                    <span className="text-xs font-semibold text-white">Filters</span>
                    {(searchQuery || selectedCountry) && (
                      <span className="w-1.5 h-1.5 bg-[#66dbe1] rounded-full"></span>
                    )}
                  </div>
                  <ChevronDown className={`w-3.5 h-3.5 text-[#ebebeb] transition-transform duration-200 ${isFiltersOpen ? 'rotate-180' : ''}`} />
                </button>
              </CollapsibleTrigger>
              <CollapsibleContent className="mt-1.5">
                <div className="flex flex-col gap-1.5">
                  {/* Search Input */}
              <div className={`flex w-full sm:w-[200px] lg:w-[180px] xl:w-[220px] items-center gap-2 ${isMobile ? 'px-1.5 py-1.5' : 'px-2 lg:px-3 py-2'} bg-[#ffffff24] rounded-md overflow-hidden border border-solid border-[#dae0e633] shadow-[0px_1px_2px_#1018280a]`}>
                <img
                  className={`relative ${isMobile ? 'w-3 h-3' : 'w-4 h-4'} flex-shrink-0`}
                  alt="Search"
                  src="/zoom-search.svg"
                />
                <Input
              ...
              <div className="relative w-full sm:w-[160px] lg:w-[150px] xl:w-[180px]" ref={countryDropdownRef}>
                <div className="relative">
                  <Input
                    ...
                    className={`bg-[#ffffff24] border border-[#dae0e633] text-[#ebebeb] text-xs lg:text-sm ${isMobile ? 'px-1.5 py-1.5 pr-6 h-[32px]' : 'px-2 lg:px-3 py-2 pr-7 h-[38px]'} placeholder:text-[#ebebeb99] focus-visible:ring-2 focus-visible:ring-[#4eb7bd]/50 w-full`}
              ...
              <Tabs value={timeRange} onValueChange={setTimeRange} className="w-full sm:w-auto flex-shrink-0">
                <TabsList className={`bg-[#ffffff14] border border-[#eaebf024] w-full sm:w-auto ${isMobile ? 'h-[32px]' : 'h-[38px]'}`}>
                  <TabsTrigger value="24h" className={`data-[state=active]:bg-[#4eb7bd] data-[state=active]:text-white text-[#ebebeb] flex-1 sm:flex-none ${isMobile ? 'text-[10px] px-1' : 'text-xs px-2 lg:px-3'}`}>
                    {t("dashboard.timeRange24h")}
                  </TabsTrigger>
                  <TabsTrigger value="7d" className={`data-[state=active]:bg-[#4eb7bd] data-[state=active]:text-white text-[#ebebeb] flex-1 sm:flex-none ${isMobile ? 'text-[10px] px-1' : 'text-xs px-2 lg:px-3'}`}>
                    {t("dashboard.timeRange7d")}
                  </TabsTrigger>
                  <TabsTrigger value="30d" className={`data-[state=active]:bg-[#4eb7bd] data-[state=active]:text-white text-[#ebebeb] flex-1 sm:flex-none ${isMobile ? 'text-[10px] px-1' : 'text-xs px-2 lg:px-3'}`}>
                    {t("dashboard.timeRange30d")}
                  </TabsTrigger>
                  <TabsTrigger value="1y" className={`data-[state=active]:bg-[#4eb7bd] data-[state=active]:text-white text-[#ebebeb] flex-1 sm:flex-none ${isMobile ? 'text-[10px] px-1' : 'text-xs px-2 lg:px-3'}`}>
        ...
        <Tabs value={activeView} onValueChange={handleTabChange} className={`${isMobile ? 'mt-2' : 'mt-6'} w-full`} style={{ maxWidth: '100%' }}>
          <div className={`overflow-x-auto -mx-3 sm:mx-0 px-3 sm:px-0 ${isMobile ? 'mb-2' : 'mb-6'}`}>
          ...
          <TabsContent value="categories" className="space-y-6 mt-0">
            <OutbreakCategories timeRange={timeRange} countryId={selectedCountry} />
          </TabsContent>
