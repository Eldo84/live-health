...
export const GenderComparisonChart = ({ title, selectedCategory }: GenderComparisonChartProps) => {
  const chartData = useMemo(() => {
    ...
    return filteredData.filter(d => d.female > 0 || d.male > 0).sort((a, b) => (b.female + b.male) - (a.female + a.male)).slice(0, 10).map(d => {
    ...
    return (
      <div className="glass rounded-lg p-2 sm:p-3 shadow-xl border border-border/50 max-w-[180px]">
        ...
        <div className="space-y-0.5 text-[10px] sm:text-xs">
          <div className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: 'hsl(320, 70%, 55%)' }} /><span>Female: {item.femalePercent}%</span></div>
          <div className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: 'hsl(199, 89%, 48%)' }} /><span>Male: {item.malePercent}%</span></div>
        </div>
      </div>
    );
  };
