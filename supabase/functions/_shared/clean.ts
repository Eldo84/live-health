...
function getSourcePriority(source: string): number {
  ...
  if (sourcePriority[source] !== undefined) {
    return sourcePriority[source];
  }
  // Check for partial match (e.g., "Google News (en)" -> "Google News")
...
export async function cleanDuplicates(
  ...
  // OPTIMIZATION: Only check signals from last 14 days - older signals won't match new articles

  // Get article IDs that have outbreak signals from the last 14 days
  ...
  // Batch the query to avoid URL length limits (max ~200 IDs per query)
  // Use larger batches to reduce roundtrips
  const BATCH_SIZE = 200;
  const totalBatches = Math.ceil(articleIdsWithSignals.length / BATCH_SIZE);
  console.log(`[CLEAN] Fetching ${articleIdsWithSignals.length} recent articles (last 14 days) in ${totalBatches} batch(es)`);
  ...
  console.log(`[CLEAN] Total existing articles fetched: ${existingArticles.length} in ${batchDuration}s`);
  ...
  const filtered = deduplicatedByTitle.filter((a) => {
    ...
    if (existingArticleUrlsSet.has(a.url)) {
      ...
      if (!a.source.includes("Google News")) {
        console.log(
