...
const CITY_SIGNAL_COOLDOWN_DAYS = 7;
const COUNTRY_SIGNAL_COOLDOWN_DAYS = 7;
...
export async function storeArticlesAndSignals({
  ...
  for (const article of matchedArticles) {
    ...
    let source = sources?.find((s: any) => {
      ...
      // Handle Google News with language suffix (e.g., "google news (en)")
      if (articleSource.startsWith("google news") && dbNameLower.includes("google news")) {
        return true;
      }
    ...
    if (!article.source.includes("Google News")) {
    ...
    if (dbCountry && article.diseases && article.diseases.length > 0) {
      for (const disease of article.diseases) {
        ...
        const lookbackIso = new Date(Date.now() - DUPLICATE_LOOKBACK_DAYS * 24 * 60 * 60 * 1000).toISOString();
        const validCityName = normalizedCityForCheck !== null && cityCoords !== null;
        const dayMs = 24 * 60 * 60 * 1000;
        let shouldSkip = false;
        
        if (validCityName && normalizedCityForCheck) {
          const { data: cityMatch } = await supabase
            .from("outbreak_signals")
            .select("id, city, latitude, longitude, detected_at")
          ...
          if (cityMatch?.detected_at) {
            const lastDetected = new Date(cityMatch.detected_at).getTime();
            if (Date.now() - lastDetected < CITY_SIGNAL_COOLDOWN_DAYS * dayMs) {
              shouldSkip = true;
            }
          }
        } else {
          ...
          if (countryMatch?.detected_at) {
            const lastDetected = new Date(countryMatch.detected_at).getTime();
            if (Date.now() - lastDetected < COUNTRY_SIGNAL_COOLDOWN_DAYS * dayMs) {
              shouldSkip = true;
            }
          }
        }
