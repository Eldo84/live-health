...
const generateWeeklyRecommendations = async (
  topDiseases: WeeklyDiseaseSummary[],
  deepseekApiKey: string | undefined,
  supabase: any
): Promise<WeeklyReportRecommendations> => {
...
Make recommendations specific, actionable, and based on the actual diseases listed. Prioritize diseases with higher case counts.`;
  ...
  // Fallback recommendations if AI generation failed or API key not available
  const needsFallback = 
  ...
  // Only use stored recommendations if they exist
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Verify this is called with service role key (from cron job)
    ...
    // For cron jobs, we expect the service role key
    ...
    const { data: topDiseases, error: summaryError } = await supabase
      .rpc<WeeklyDiseaseSummary>("get_weekly_top_diseases");

    if (summaryError) {
      console.error("Error calling get_weekly_top_diseases:", summaryError);
      throw new Error(`Failed to compute weekly top diseases: ${summaryError.message}`);
    }

    const diseases = topDiseases || [];

    // 2) Only send notifications if there are diseases
    if (diseases.length === 0) {
      console.log("No diseases found in the last 7 days. Skipping report generation and notification creation.");
      return new Response(
        JSON.stringify({
          success: true,
          message: "No diseases found in the last 7 days. Notifications not sent.",
    ...
    // 3) Generate recommendations for the weekly report
    console.log("Generating weekly report recommendations...");
    const recommendations = await generateWeeklyRecommendations(diseases, deepseekApiKey, supabase);

    // 4) Calculate week dates (last 7 days)
    const now = new Date();
    const weekEndDate = new Date(now);
    weekEndDate.setHours(0, 0, 0, 0); // Start of today
    const weekStartDate = new Date(weekEndDate);
    weekStartDate.setDate(weekStartDate.getDate() - 6); // 7 days ago (including today)

    // 5) Store the weekly report in database
    ...
    const { error: storeError } = await supabase
      ...
      .upsert(
        {
          report_date: reportDate.toISOString().split('T')[0], // YYYY-MM-DD format
    ...
    if (storeError) {
      console.error("Error storing weekly report:", storeError);
      throw new Error(`Failed to store weekly report: ${storeError.message}`);
    }

    console.log("Weekly report stored successfully in database");
    ...
    if (allUsers.users.length === 0) {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: "No users found. Notifications not sent.",
          notificationsCreated: 0,
          reportStored: true,
        }),
        {
    ...
    const topDiseaseNames = diseases.slice(0, 3).map(d => d.disease_name).join(", ");
    const diseaseCount = diseases.length;
    const title = "Weekly Top Diseases Report";
    const message = diseaseCount === 1
      ? `Top disease this week: ${topDiseaseNames}. View the full report for details.`
      : `Top ${Math.min(diseaseCount, 3)} diseases this week: ${topDiseaseNames}${diseaseCount > 3 ? ` and ${diseaseCount - 3} more` : ""}. View the full report for details.`;

    // 8) Create notifications for all users
    const notifications = allUsers.users.map((user) => ({
      user_id: user.id,
      type: "weekly_top_diseases",
      title: title,
      message: message,
      action_url: "/app/dashboard/weekly-report",
      action_label: "View Weekly Report",
      priority: "normal",
      read: false,
    }));
    ...
    console.log(`Successfully created ${notifications.length} weekly top diseases notifications for ${diseases.length} diseases`);

    return new Response(
      JSON.stringify({
        success: true,
        message: `Weekly top diseases notifications sent to ${notifications.length} users`,
        notificationsCreated: notifications.length,
        diseasesFound: diseases.length,
        topDiseases: diseases.slice(0, 5).map(d => d.disease_name),
        reportStored: true,
      }),
      {
        status: 200,
    ...
    console.error("Error sending weekly top diseases notifications:", error);
    return new Response(
